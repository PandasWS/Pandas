...
bool npc_event_is_filter(enum npce_event eventtype) {
	static std::vector<enum npce_event> filter_npce = {
		...
		NPCF_CASHSHOPOPEN,	// cashshop_open_filter_name	// OnPCCashShopOpenFilter		// 打开商城前触发过滤器 [聽風]
#endif // Pandas_NpcFilter_CASHSHOPOPEN

#ifdef Pandas_NpcFilter_CASHSHOPBUY
		NPCF_CASHSHOPBUY,	// cashshop_buy_filter_name	// OnPCCashShopBuyFilter		// 点击购买按钮后还没有扣除CASH点数和拿到物品前触发过滤器 [聽風]
...
const char *npc_get_script_event_name(int npce_index)
{
	...
	case NPCF_CASHSHOPOPEN:
		return script_config.cashshop_open_filter_name;	// OnPCCashShopOpenFilter		// 打开商城前触发过滤器 [聽風]
#endif // Pandas_NpcFilter_CASHSHOPOPEN

#ifdef Pandas_NpcFilter_CASHSHOPBUY
	case NPCF_CASHSHOPBUY:
		return script_config.cashshop_buy_filter_name;	// OnPCCashShopBuyFilter		// 点击购买按钮后还没有扣除CASH点数和拿到物品前触发过滤器 [聽風]
#endif // Pandas_NpcFilter_CASHSHOPBUY
	...
	case NPCE_CASHSHOPBUY:
		return script_config.cashshop_buy_event_name;	// OnPCCashShopBuyEvent		// 商城购买成功后触发事件 [聽風]
#endif // Pandas_NpcEvent_CASHSHOPBUY

#ifdef Pandas_NpcEvent_CASHSHOPCLOSE
	case NPCE_CASHSHOPCLOSE:
		return script_config.cashshop_close_event_name;	// OnPCCashShopCloseEvent		// 关闭商城后触发事件 [聽風]
