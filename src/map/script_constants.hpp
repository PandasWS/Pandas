...
#define SCRIPT_CONSTANTS_HPP
	...
	export_constant(NPCF_CASHSHOPOPEN);	// cashshop_open_filter_name	// OnPCCashShopOpenFilter		// 打开商城前触发过滤器 [聽風]
#endif // Pandas_NpcFilter_CASHSHOPOPEN

#ifdef Pandas_NpcFilter_CASHSHOPBUY
	export_constant(NPCF_CASHSHOPBUY);	// cashshop_buy_filter_name	// OnPCCashShopBuyFilter		// 点击购买按钮后还没有扣除CASH点数和拿到物品前触发过滤器 [聽風]
	...
	export_constant(NPCE_CASHSHOPBUY);	// cashshop_buy_event_name	// OnPCCashShopBuyEvent		// 商城购买成功后触发事件 [聽風]
#endif // Pandas_NpcEvent_CASHSHOPBUY

#ifdef Pandas_NpcEvent_CASHSHOPCLOSE
	export_constant(NPCE_CASHSHOPCLOSE);	// cashshop_close_event_name	// OnPCCashShopCloseEvent		// 关闭商城后触发事件 [聽風]
