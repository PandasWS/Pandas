...
#else
struct mob_data* mob_once_spawn_sub(struct block_list* bl, int16 m, int16 x, int16 y, const char* mobname, int32 mob_id, const char* event, uint32 size, enum mob_ai ai, uint16 spawn_flag)
...
#else
		md = mob_once_spawn_sub((sd) ? &sd->bl : nullptr, m, x, y, mobname, c, event, size, ai, spawn_flag);
#endif // Pandas_FuncDefine_Mob_Once_Spawn
...
#else
int32 mob_dead(struct mob_data *md, struct block_list *src, int32 type, uint16 skill_id)
...
#ifdef Pandas_NpcExpress_MOBDROPITEM
			if (md && !npc_express_aide_mobdropitem(md, src, dlist, entry->nameid, drop_rate, 1))
				continue;
#endif // Pandas_NpcExpress_MOBDROPITEM
...
#ifdef Pandas_NpcExpress_MOBDROPITEM
				if (md && npc_express_aide_mobdropitem(md, src, dlist, mobdrop->nameid, mobdrop->rate, 2))
#endif // Pandas_NpcExpress_MOBDROPITEM
...
#ifdef Pandas_NpcExpress_MOBDROPITEM
				if (!npc_express_aide_mobdropitem(md, src, (mvp_sd ? mvp_sd->bl.id : 0), entry->nameid, temp, 5))
					continue;
#endif // Pandas_NpcExpress_MOBDROPITEM
...
#else
		mvptomb_create(md, mvp_sd != nullptr ? mvp_sd->status.name : (first_sd != nullptr ? first_sd->status.name : nullptr), time(nullptr), mvp_sd ? mvp_sd->bl.id : 0);
#endif // Pandas_FuncParams_Mob_MvpTomb_Create
