...
int32 chmapif_bonus_script_get(int32 fd) {
	if (RFIFOREST(fd) < 6)
		...
		if (SQL_ERROR == stmt.Prepare(
			...
			schema_config.bonus_script_db, cid, MAX_PC_BONUS_SCRIPT) ||
			SQL_ERROR == stmt.Execute() ||
			SQL_ERROR == stmt.BindColumn(0, SQLDT_STRING, &tmp_bsdata.script_str, sizeof(tmp_bsdata.script_str), NULL, NULL) ||
			SQL_ERROR == stmt.BindColumn(1, SQLDT_INT64, &tmp_bsdata.tick, 0, NULL, NULL) ||
			SQL_ERROR == stmt.BindColumn(2, SQLDT_UINT16, &tmp_bsdata.flag, 0, NULL, NULL) ||
			SQL_ERROR == stmt.BindColumn(3, SQLDT_UINT8, &tmp_bsdata.type, 0, NULL, NULL) ||
			SQL_ERROR == stmt.BindColumn(4, SQLDT_INT16, &tmp_bsdata.icon, 0, NULL, NULL) ||
			SQL_ERROR == stmt.BindColumn(5, SQLDT_UINT64, &tmp_bsdata.bonus_id, 0, NULL, NULL)			// 绑定了一个 ID 字段
			)
		{
			SqlStmt_ShowDebug(stmt);
			return 1;
