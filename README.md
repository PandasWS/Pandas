...
能接近官方的模拟器 ( 意味着其改动依据都将以 kRO 为准；而所有非官方的改动都难以通过 ) 。
...
推荐的配置信息。特别是硬盘部分，有条件的同学一定要换成 SSD 固态硬盘！
...
-   获取熊猫模拟器源代码 ( 推荐使用 Git 克隆而不是直接下载源代码，因为会丢失仓库信息 )
...
CMake 编译方式，对于这一点有相关经验的同学务必注意一下，避免被坑。在本例子中，我们将使用
...
不通的萌新玩家，毕竟搞懂一个开源项目需要大量的背景知识当支撑。我们亦希望通过早期维持使用
难度以便召集一群知识覆盖面相当，有经验的使用者。为以后熊猫模拟器的迭代做储备。

### 当前对应仙境传说的哪个 EP 版本？

此问题无法答复，因为模拟器并非官方服务端，我们游戏中的所有功能都是「尾随」官方之后通过一些
爱好者在官服的体验游玩，分析和观察后模拟实现的；官服的每个 EP 版本都是无数小调整组合在
一起的 「集合包」，这些调整中有的模拟起来比较简单而有的功能模拟器来非常复杂，这就导致了
模拟器实装这些改动的时间的无法被统一。
...
在 `2020` 年之后的客户端中，虽然有非常多的新特效和功能，但是其中有些功能并没有被模拟器
很好的支持，因此可能会在使用的过程中发现一些功能的缺失；不过别担心，这些功能也不影响玩家的
主要操作，若您真的希望客户端封包使用 `20200409`，那么只需下载源代码后通过修改
...
对这么大量的文本进行汉化和校对不是简单的「懂英文」就搞得定的，特别、极其费时费力！
...
### 主要贡献者
- [HongShin] —— 积极的发现了非常多的缺陷和故障，使得项目的稳定性、健壮性得以提升
