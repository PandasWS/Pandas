...
/*
- 身上同一种类的护身符有几个，它的“使用脚本”就会被触发几次
- 当身上同一种类的护身符数量有变化时，如果它有“使用脚本”，那么会触发重算角色的能力值（以便使玩家能受到最新数量的护身符影响）
- 同一种类的护身符无论多少，只有第一个放到背包的时候会触发且只触发一次“穿戴脚本”
- 同一种类的护身符无论多少，只有最后一个从背包移除掉的时候回触发且只触发一次“卸装脚本”
- 若在“使用脚本”中包含会导致重算角色能力值的指令（比如 sc_start 系列指令）时，应设法避免递归情况出现（好刀上报的BUG因此引起）
- 理想情况下，在“使用脚本”中使用会导致重算角色能力值的指令（比如 sc_start 系列指令），不应该导致地图服务器崩溃
*/
