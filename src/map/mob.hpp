...
#else
struct mob_data* mob_once_spawn_sub(struct block_list* bl, int16 m, int16 x, int16 y, const char* mobname, int mob_id, const char* event, uint32 size, enum mob_ai ai, uint16 spawn_flag = 0);
...
#else
int mob_once_spawn(map_session_data* sd, int16 m, int16 x, int16 y,
	const char* mobname, int mob_id, int amount, const char* event, uint32 size, enum mob_ai ai, uint16 spawn_flag = 0);
